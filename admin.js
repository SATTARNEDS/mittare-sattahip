...
document.getElementById("login-form").addEventListener("submit", async event => {
  ...
  try {
    const data = await api("/api/admin/login", { method: "POST", body: { username: form.get("username"), password: form.get("password") } });
    state.user = data.user;
    state.csrfToken = data.csrfToken;
    showAdmin();
    await refreshWorkspace();
...
async function loadQuestions() {
  ...
  state.questions = await api(`/api/admin/questions?${parameters}`);
  renderQuestions();
  renderTopicList();
}
...
questionForm.addEventListener("submit", async event => {
  ...
  try {
    await api(id ? `/api/admin/questions/${id}` : "/api/admin/questions", { method: id ? "PUT" : "POST", body: payload });
...
document.getElementById("questions-body").addEventListener("click", async event => {
  ...
  if (button.dataset.action === "reject") {
    ...
    if (!reason) return;
    try { await api(`/api/admin/questions/${id}/reject`, { method: "POST", body: { reason } }); showToast("ส่งข้อสอบกลับไปแก้ไขแล้ว"); await refreshWorkspace(); } catch (error) { showToast(error.message); }
    return;
  }
  ...
  try {
    await api(`/api/admin/questions/${id}/${button.dataset.action}`, { method: "POST" });
...
async function showHistory(id) {
  const versions = await api(`/api/admin/questions/${id}/versions`);
...
document.getElementById("user-form").addEventListener("submit", async event => {
  ...
  try {
    await api("/api/admin/users", { method: "POST", body: Object.fromEntries(form) });
