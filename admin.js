...
function renderAttemptCard(attempt) {
  ...
  return `<details class="attempt-card"><summary><span class="attempt-summary-main"><span class="badge badge-${result.passed ? "published" : "paused"}">${escapeHtml(result.label)}</span><span><strong>${percentage}%</strong><small>${attempt.score}/${attempt.total_questions} คะแนน • ${attemptModeLabels[attempt.exam_mode] || "ฝึกข้อสอบ"}</small></span></span><span class="attempt-summary-meta">${escapeHtml(formatDate(attempt.completed_at))}</span></summary><div class="attempt-detail"><p class="attempt-criteria ${result.passed ? "is-passed" : "is-failed"}">${escapeHtml(criteria)}</p><dl><div><dt>ชุดที่ทำ</dt><dd>${escapeHtml(attempt.selected_topic)}</dd></div><div><dt>เวลา</dt><dd>${Math.floor(attempt.duration_seconds / 60)} นาที ${attempt.duration_seconds % 60} วินาที</dd></div></dl>${topicDetails ? `<h3>คะแนนแยกรายหมวด</h3><ul>${topicDetails}</ul>` : "<p>ไม่มีข้อมูลคะแนนแยกรายหมวด</p>"}</div></details>`;
}
