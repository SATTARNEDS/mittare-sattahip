...
$env:ADMIN_LINE_USER_ID="LINE User ID ผู้ดูแล ถ้าต้องการเปลี่ยนจากค่าเริ่มต้น"
python app.py
```
...
LINE Push ใช้ LINE Messaging API และต้องมี `LINE User ID ผู้ดูแล` ที่เพิ่มเพื่อนกับ LINE OA แล้ว ไม่สามารถส่งด้วย LINE ID, ชื่อบัญชี หรือ `@` username ได้โดยตรง ค่าเริ่มต้นของระบบตั้งไว้เป็นผู้ดูแลที่กำหนดใน `app.py` และไม่เปิดให้แก้จากหน้า Dashboard
...
$env:ADMIN_LINE_USER_ID="ใส่ LINE User ID ผู้ดูแล ถ้าต้องการ override ค่าเริ่มต้น"
```

5. กด `ส่งแจ้งเตือนผู้ดูแล` เพื่อส่งสรุปงานหมดอายุ/ใกล้หมดอายุ/นัดติดตามไปยังผู้ดูแล
6. หรือเลือกกรมธรรม์หนึ่งรายการ แล้วกด `ส่งให้ผู้ดูแล` เพื่อส่งข้อความเฉพาะรายการนั้น

ถ้ายังไม่พร้อมใช้ Push ให้ใช้ปุ่ม `คัดลอกข้อความ` หรือ `เปิด LINE` เพื่อส่งเองก่อนได้
...
8. ระบบจะบันทึก event ไว้ในฐานข้อมูลเพื่อใช้ตรวจสอบทางเทคนิค แต่ Dashboard จะไม่แสดงรายการ User ID ที่ทักเข้ามาแล้ว
...
5. กรอกอย่างใดอย่างหนึ่ง:

```text
เบอร์โทร: 081-111-2222
หรือ เลขอ้างอิง/เลขกรมธรรม์: MT4-DEMO-001
```
